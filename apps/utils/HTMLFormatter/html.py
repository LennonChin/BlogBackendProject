...
class HtmlFormatter(Formatter):
    ...
    `linenos`
        ...
        is *new in Pygments 0.8*). If set to ``'ul'``, the code line will be
        integrated in the ``<ul>`` tag that developer can specify custom css
        to show line numbers by using  `list-style-type` ul and li.
    ...
    **Subclassing the HTML formatter**
    ...
    def _wrap_ullinenos(self, inner):
        ...
        for t, line in inner:
            if t:
                if aln:
                    li_start = '<li id="#%s-%d">' % (la, lncount)
                lncount += 1
            dummyoutfile.write(li_start + line + '</li>')
        lines = []
        ...
        # some configurations seem to mess up the formatting...
        if nocls:
            yield 0, ('<ul class="%stable">' % self.cssclass + ls)
        else:
            yield 0, ('<ul class="%stable">' % self.cssclass + ls)
        yield 0, dummyoutfile.getvalue()
        yield 0, '</ul>'
